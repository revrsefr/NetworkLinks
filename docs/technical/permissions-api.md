...
Permissions were introduced in NetLink 1.0 as a way for plugins to manage command access, replacing the old `irc.checkAuthenticated()`. The permissions system in NetLink is fairly simple, globally assigning a list of permissions to each hostmask/exttarget.
...
The permissions module is available as `netlink.coremods.permissions`. Usually, plugins import it this format:
...
`utils.NotAuthorizedError` can be treated like any other exception, so it's possible to wrap it around `try:` / `except:` for more complex access checking ([example in the Automode plugin](https://github.com/revrsefr/NetworkLinks/blob/1.1.1/plugins/automode.py#L64-L68)).
...
Example of this in [Automode](https://github.com/revrsefr/NetworkLinks/blob/1.1-alpha1/plugins/automode.py#L38-L39):
