# Writing plugins for NetLink

***Last updated for 2.1-alpha2 (2019-06-27).***

Most features in NetLink (Relay, Automode, etc.) are implemented as plugins, which can be mix-and-matched on any particular instance. Without any plugins loaded, NetLink can connect to servers but won't accomplish anything useful.

This guide, along with the sample plugin [`example.py`](../../plugins/example.py) aim to show the basics of writing plugins for NetLink.
...
4) A freeform `dict` of arguments, where data keys vary by command - see the [NetLink hooks reference](hooks-reference.md) for what's available where.
...
As of NetLink 2.0, the return value of hook handlers are used to determine how the original event will be passed on to further handlers (that is, those created by plugins loaded later, or hook handlers registered with a lower priority).
...
Hook handlers may raise exceptions without blocking the event from reaching further handlers; these are caught by NetLink and logged appropriately.

#### Modifying a hook payload

As of NetLink 2.1, it is acceptable to modify a hook event payload in any plugin handler. This can be used for filtering purposes, e.g. Antispam's part/quit message filtering.
...
| `relay`           | PRIVMSG, NOTICE | 200      | Fixes https://github.com/revrsefr/NetworkLinks/issues/123. Essentially, this lets Relay forward messages calling commands before letting the command handler work (and then relaying its responses). |
...
Plugins can also define service bot commands, either for the main NetLink service bot or for one created by the plugin itself. This section only details the former - see the [Services API Guide](services-api.md) for details on the latter.

Commands are registered by calling `utils.add_cmd()` with one or two arguments. Ex)
- `utils.add_cmd(testcommand, "hello")` registers a function named `testcommand` as the command handler for `hello` (i.e. `/msg NetLink hello`)
...
- **args**: A `list` of command arguments (not including the command name) that the command was called with. For example, `/msg NetLink hello world 1234` would give an `args` list of `['world', '1234']`

As of NetLink 1.2, there are two ways for a plugin to parse arguments: as a raw list of strings, or with `utils.IRCParser` (an [argparse](https://docs.python.org/3/library/argparse.html) wrapper). `IRCParser()` is documented in the ["using IRCParser"](using-ircparser.md) page.
...
These functions are called in the form: `irc.command(arg1, arg2, ...)`. For example, the command `irc.join('10XAAAAAB', '#bots')` would join a NetLink client with UID `10XAAAAAB` to the channel `#bots`.
...
The sender UID for all of these can be set using the `source` argument, and defaults to the main NetLink client.
...
- `main(irc=None)`: Called on plugin load. `irc` is only defined when the plugin is being reloaded from a network: otherwise, it means that NetLink has just been started.
...
Use NetLink's [global logger](https://docs.python.org/3/library/logging.html) (`from netlink.log import log`) instead of print statements.
...
`classes.py`, `utils.py` and `structures.py` all provide a ton of public methods which aren't documented here for conciseness. In `classes.py`, `NetLinkNetworkCore` and `NetLinkNetworkCoreUtils` (which all protocol modules inherit from) are where many utility and state-checking functions sit.
