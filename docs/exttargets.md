...
**Extended targets** or **exttargets** extend regular hostmask matching by checking users against specific conditions. NetLink exttargets are supported by most plugins in the place of `nick!user@host` masks (provided they use `IRCNetwork.match_host()` as their backend).

Exttargets were introduced in NetLink 0.9 alongside [Automode](automode.md), with the goal of making user/ACL matching more versatile. As of NetLink 2.0, the following exttargets are supported:

### The "$account" target (NetLink 0.9+)
...
### The "$channel" target (NetLink 0.9+)
...
### Target inversion (NetLink 1.2+)
In NetLink 1.2 and above, all targets and hostmasks can be inverted by placing a `!` before the target:
...
For users on NetLink version **before 1.2**, target inversion is *only* supported with exttargets (i.e. `!$account` will work, but not `!*!*@localhost`.

### The "$network" target (NetLink 1.2+)
...
### The "$netlinkacc" target (NetLink 0.9+)
Used to match users logged in to *NetLink* (i.e. via the `identify` command). **As of NetLink 2.0, The "$netlink:" prefix is implied if you specify a NetLink account name without it.**

- `$netlinkacc` -> Returns True if the target is logged in to NetLink.
- `$netlinkacc:accountname` -> Returns True if the target's NetLink login matches the one given (case insensitive).

### The "$realname" target (NetLink 2.0+)
Used to match users with certain realnames.

- `$realname:*James*`: matches anyone with "James" in their real name (case insensitive).

### The "$service" target (NetLink 2.0+)
