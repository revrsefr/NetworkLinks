...
How is this relevant to an operator? It means that you **cannot ban users** using banmasks such as `*/net1!*@*`! The nick suffix is something NetLink adds artificially; on `net1`'s IRCd, which check the bans locally, the nick suffix doesn't exist and will therefore *not* match anyone.

### Services compatibility

While NetLink is generally able to run independently of individual networks' services, there are some gotchas. This list briefly details services features that have been known to cause problems with Relay. **Using any of these features in conjunction with Relay is *not* supported.**
...
- *Any*: **Do NOT register a relayed channel on multiple networks**
    ...
    - Clientbot is an exception to this, though you may want to add Clientbot networks to CLAIM so that NetLink doesn't try to reverse modes set by services on the Clientbot network.
- *Any*: **Do NOT jupe virtual Relay servers** (e.g. `net.relay`)
    - Rationale: This will just make NetLink split off - you should instead [delink any problem networks / channels](#dealing-with-disputes-and-emergencies).
- Multiple NetLink Relay instances:
    - **Do NOT connect a network twice to any NetLink instance**.
    - **Do NOT connect a network to 2+ separate NetLink instances if there is another network already acting as a hub for them**.
...
The basic steps for setting up a relay is to first CREATE the channel with NetLink on the network that owns it, and run LINK from each network that wants to link to it. In most cases, you want to run CREATE on the network where the channel is registered with services.
...
- `/msg NetLink create #channelname`
...
- `/msg NetLink link othernet #channelname`
...
- `/msg NetLink link othernet #lobby #othernet-lobby`

To completely remove a relay channel (on the network that created it):
- `/msg NetLink destroy #channelname`

To delink a channel *linked to another network*:
- `/msg NetLink delink #localchannelname`

To delink one of *your* channels from another network:
- `/msg NetLink delink #yourchannelname <name-of-other-network>`
...
LINKACL allows you to allow or deny networks from linking to your channel. New channels are created using a blacklist by default, though this can be configured via the [`relay::linkacl_use_whitelist` option](https://github.com/revrsefr/NetworkLinks/blob/3.0.0/example-conf.yml#L823-L826).

To change between blacklist and whitelist mode:
- `/msg NetLink linkacl whitelist #channel true/false`
...
- `/msg NetLink linkacl #channel list`

To add a network to the whitelist **OR** remove a network from the blacklist:
- `/msg NetLink linkacl #channel allow goodnet`

To remove a network from the whitelist **OR** add a network to the blacklist:
- `/msg NetLink linkacl #channel deny badnet`

### Adding channel descriptions

Starting with NetLink 2.0, you can annotate your channels with a description to use in LINKED:

To view the description for a channel:
- `/msg NetLink chandesc #channel`

To change the description for a channel:
- `/msg NetLink chandesc #channel your text goes here`
...
Special kill handling was introduced in NetLink 2.0, while in previous versions they were always bounced:
...
2) If the target and source networks share a [kill share pool](https://github.com/revrsefr/NetworkLinks/blob/3.0.0/example-conf.yml#L782-L792), relay the kill as-is.
...
Network bans are purposely not supported; see https://github.com/revrsefr/NetworkLinks/issues/521#issuecomment-352316396.

### Delinking channels

To delink another network from a channel your network owns:

- `/msg NetLink delink #yourchannel badnetwork`

To delink your network from a bad network's channel:

- `/msg NetLink delink #badchannel`

Basically, only one of the two above commands will work for one specific channel. Almost always, the network that owns a channel should be the one who has it registered via their services. You can see a list of channels by typing `/msg NetLink linked`.
