...
### I get errors like "ImportError: No module named 'yaml'" when I start NetLink

You are missing dependencies - re-read https://github.com/revrsefr/NetworkLinks/blob/master/README.md#installation
...
This likely indicates an indentation issue. When you create a list in YAML (NetLink's config format), all entries must be indented consistently. For example, this is **bad**:
...
A common misconception is that the YAML format is something specific to Python or NetLink, but this is not the case! YAML is a programming language-independent standard which *happens* to use indents for structures like Python does, but [parsers for it exist just about everywhere](http://yaml.org/).
...
### NetLink won't connect to my network!
...
- Is the target network's IRCd showing failed connection attempts?
    - If not:
        1) Is NetLink connecting to the right port (i.e. one the IRCd is listening on?)
...
See https://github.com/revrsefr/NetworkLinks/issues/463 - the problem appears to be caused somewhere in Python's SSL stack and/or OpenSSL, and not directly by our code.
...
### I turned autoconnect for NetLink on, and now I'm getting errors!

NetLink does not support inbound connections - much like regular services such as Atheme or Anope, it only connects outwards *to* IRCds. (If you don't understand what this means, it means you should turn autoconnect **off** for NetLink)

## Relay issues

### Does everyone need to install NetLink Relay for it to work?

**No!** Only the NetLink administrator needs to host a NetLink instance with the `relay` plugin loaded, as each instance can connect to multiple networks. Everyone else only needs to add a link block on their IRCd.

InterJanus-style links between NetLink daemons are not supported yet; see https://github.com/revrsefr/NetworkLinks/issues/99 for any progress regarding that.

### What are NetLink's advantages over Janus?

NetLink provides, in no particular order:
...
First, check whether the SQUIT message includes the nick that triggered the netsplit. If this nick includes any characters not allowed in regular IRC, such as the slash ("/"), or is otherwise an invalid nick (e.g. beginning with a hyphen or number), this likely indicates a bug in NetLink Relay. These problems should be reported on the issue tracker.

However, if the nick mentioned is legal on IRC, this issue is likely caused by a max nick length misconfiguration: i.e. the Relay server is introducing nicks too long for the target network. This can be fixed by setting the `maxnicklen` option in the affected network's NetLink `server:` block to the same value as that network's `005` `NICKLEN` (that is, the `NICKLEN=<num>` value in `/raw version`).

### Clientbot doesn't relay both ways!

Load the `relay_clientbot` plugin. https://github.com/revrsefr/NetworkLinks/blob/1.3-beta1/example-conf.yml#L465-L468

### How do I turn off colors in Clientbot?
See https://github.com/revrsefr/NetworkLinks/blob/master/docs/advanced-relay-config.md#custom-clientbot-styles, especially the section "Disabling Colors/Control Codes".

### Relay is occasionally dropping users from channels!

This usually indicates a serious bug in either Relay or NetLink's protocol modules, and should be reported as an issue. When asking for help, please state which IRCds your NetLink instance is linking to: specifically, which IRCd the missing users are *from* and which IRCd the users are missing *on*. Also, be prepared to send debug logs as you reproduce the issue!
- Another tip in debugging this is to run `showchan` on the affected channels. If NetLink shows users in `showchan` that aren't in the actual user list, this is most likely a protocol module issue. If `showchan`'s output is correct, it is instead probably a Relay issue where users aren't spawning correctly.

### Does Relay support mode +R, +M, etc.? How does Relay handle modes supported on one IRCd but not on another?
Essentially, NetLink maps IRCd modes together by name, so modes that use different characters on different IRCds can be recognized as the same "mode". Tables of supported channel modes, user modes, and extbans (in 2.0+) can be found at https://github.com/revrsefr/NetworkLinks/tree/devel/docs/modelists. Note that third party/contrib modules implementing modes are generally *not* tested / supported.

Relay in particular uses whitelists to determine which modes are safe to relay: for 2.0.0, this is https://github.com/revrsefr/NetworkLinks/blob/71a24b8/plugins/relay.py#L903-L969. **Most *channel* modes recognized by NetLink are whitelisted and usable with Relay**, with the following exceptions:
...
See https://github.com/revrsefr/NetworkLinks/blob/devel/docs/relay-quickstart.md#kill-handling

### How does Relay handle KLINE/GLINE/ZLINE?

It doesn't. https://github.com/revrsefr/NetworkLinks/issues/521#issuecomment-352316396 explains my reasons for skipping over this:
...
### Service bots aren't spawning on my network, even though NetLink connects

This indicates either a bug in NetLink's protocol module or (less commonly) a bug in your IRCd. Hint: ENDBURST is likely not being sent or received properly, which causes service bot spawning to never trigger.

Make sure you're using an [officially supported IRCd](https://github.com/revrsefr/NetworkLinks#supported-ircds) before requesting help, as custom IRCd code can potentially trigger S2S bugs and is not something we can support.
