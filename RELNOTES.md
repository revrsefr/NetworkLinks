...
# NetLink 3.1-beta1 (2021-12-30)

### Feature changes

- **NetLink now requires Python >= 3.7**
- **protocols/inspircd now defaults to InspIRCd 3.x mode** (`target_version=insp3`)
- **Default to system IPv4/IPv6 preference when resolving hostnames.** For existing users, this means that NetLink will most likely **default to IPv6** when resolving hostnames if your server supports it!
...
- Implement path configuration for NetLink data files (#659)
...
# NetLink 3.0.0 (2020-04-11)

Changes since 3.0-rc1:

- Added new install instructions via Docker
- global: fix type-safety errors when connected to netlink-discord
- Various code cleanup

For a broader summary of changes since 2.0.x, consult the below release notes for NetLink 3.0-rc1.

# NetLink 3.0-rc1 (2020-02-22)

NetLink 3.0 brings NetLink up to date with the latest IRCds (InspIRCd 3, UnrealIRCd 5), and introduces Discord integration via the [netlink-discord](https://github.com/NetLink/netlink-discord) contrib module. It also improves support for Unicode nicks in Relay and Clientbot.

## Major changes since NetLink 2.0.x
...
- **Added support for InspIRCd 3 and UnrealIRCd 5.x**:
  ...
  - For UnrealIRCd 5 support, no changes in NetLink are necessary.
...
- Relay now tracks kill/mode/topic clashes and will stop relaying when it detects such a conflict ([issue#23](https://github.com/jlu5/NetLink/issues/23))
- Changehost now supports network-specific configuration ([issue#611](https://github.com/jlu5/NetLink/issues/611)) and listening for services account changes - this allows for consistent account based hostmasks for SASL gateways, etc.
- Clientbot: added an option to continuously rejoin channels the bot is not in. [issue#647](https://github.com/jlu5/NetLink/issues/647)
- Antispam: added optional quit/part message filtering ([issue#617](https://github.com/jlu5/NetLink/issues/617))

### API changes
- **API Break:** Channel, Server, and User keys may now be type `int` - previously, they were always strings.
- **API Break:** NetLink now supports having multiple UIDs mapping to the same nick, as this is common on platforms like Discord.
...
- Added `utils.match_text()`, a general (regex-based) glob matcher to replace `ircmatch` calls ([issue#636](https://github.com/jlu5/NetLink/issues/636)).
- Editing hook payloads is now officially supported in plugin hook handlers ([issue#452](https://github.com/jlu5/NetLink/issues/452))
...
This branch was previously known as NetLink 2.1. For more detailed changes between 3.0-rc1 and individual 2.1 snapshots, see their separate changelogs below.


## Changes since NetLink 2.1-beta1

### Feature changes
- Added a Dockerfile for NetLink, thanks to @jameswritescode. Official images will be available on Docker Hub before the final 2.1 release.
...
- inspircd: negotiate casemapping setting on link for InspIRCd 3. [issue#654](https://github.com/jlu5/NetLink/issues/654)
...
- clientbot: added an option to continuously rejoin channels the bot is not in. [issue#647](https://github.com/jlu5/NetLink/issues/647)
- changehost: added support for network-specific options. [issue#611](https://github.com/jlu5/NetLink/issues/611)
...
- Relay now merges together network specific `clientbot_styles` options with global settings, instead of ignoring the latter if the network specific options are set. [issue#642](https://github.com/jlu5/NetLink/issues/642)
- ts6: add support for hiding NetLink servers (`(H)` in server description)
...
- More fixes for InspIRCd 3 support:
  ...
  - Fix incorrect handling of PING to NetLink subservers - this caused harmless but confusing "high latency" warnings
...
- automode: disable on networks where IRC modes aren't supported. [issue#638](https://github.com/jlu5/NetLink/issues/638)
...
- Add `get_service_options` method to merge together global & local network options. [issue#642](https://github.com/jlu5/NetLink/issues/642)
...
# NetLink 2.1-alpha2 (2019-07-14)

**NetLink now requires Python 3.5 or later!**

This release includes all changes from NetLink 2.0.3, plus the following:

### Feature changes
- **Added cachetools as a core runtime dependency** (relay and expiringdict)
- **Added beta support for InspIRCd 3** ([issue#644](https://github.com/jlu5/NetLink/issues/644)):
...
- **Removed dependencies on ircmatch** ([issue#636](https://github.com/jlu5/NetLink/issues/636)) **and expiringdict** ([issue#445](https://github.com/jlu5/NetLink/issues/445))
- Increased Passlib requirement to 1.7.0+ to remove deprecated calls
    - netlink-mkpasswd: use `hash()` instead of `encrypt()`
- Relay updates:
    - Relay now tracks kill/mode/topic clashes and will stop relaying when it detects such a conflict ([issue#23](https://github.com/jlu5/NetLink/issues/23))
...
- commands: add a `shownet` command to show server info ([issue#578](https://github.com/jlu5/NetLink/issues/578))
- antispam: added optional quit/part message filtering ([issue#617](https://github.com/jlu5/NetLink/issues/617))
...
- relay: fix KILL message formatting (regression from [issue#520](https://github.com/jlu5/NetLink/issues/520))
- relay: consistency fixes when handling hideoper mode changes ([issue#629](https://github.com/jlu5/NetLink/issues/629))
- exttargets: coerse services_account to string before matching ([issue#639](https://github.com/jlu5/NetLink/issues/639))

### Internal improvements
- **API Break:** Reworked `NetLinkNetworkCore.nick_to_uid()` specification to support duplicate nicks and user filtering
...
- Added `utils.match_text()`, a general glob matching function to replace ircmatch calls ([issue#636](https://github.com/jlu5/NetLink/issues/636))
- Editing hook payloads is now officially supported in plugin hook handlers ([issue#452](https://github.com/jlu5/NetLink/issues/452))
...
# NetLink 2.1-alpha1 (2019-05-02)

This release focuses on internal improvements to better integrate with [netlink-discord](https://github.com/NetLink/netlink-discord). It includes all fixes from 2.0.2, plus the following:
...
- Various Relay improvements:
    ...
    - Add protocol-level support for hiding users in Relay - this is used by netlink-discord to optionally hide invisible/offline users
...
- clientbot: fix /names parsing errors on networks supporting colors in hosts. [issue#641](https://github.com/jlu5/NetLink/issues/641)
...
#### Internal improvements
- log: use netlink as logger name; this prevents other libraries' debug output from making it to the NetLink log by default
...
- changehost: only send a host change if new host != original
- relay: fix inconsistent handling of the hideoper setting. [issue#629](https://github.com/jlu5/NetLink/issues/629)
- unreal: work around a potential race when sending kills on join

# NetLink 2.0.2 (2019-03-31)
...
#### Bug fixes
- clientbot: fix desync when the bot is told to kick itself ([issue#377](https://github.com/jlu5/NetLink/issues/377))
- launcher: fix PID files not being read if psutil isn't installed
- relay_clientbot no longer relays list modes set during a server burst ([issue#627](https://github.com/jlu5/NetLink/issues/627))
...
# NetLink 2.0.0 (2018-07-31)

Changes since 2.0-rc1:

- Fixed various bugs affecting Relay and Clientbot:
    - handlers: don't delete the NetLink service UID if it leaves all cahnnels
...
For a summary of changes since 1.3.x, consult the release notes for NetLink 2.0-rc1 below.

# NetLink 2.0-rc1 (2018-07-18)

NetLink 2.0 comes with a ton of new features, refinements, and optimizations. Here is a summary of the most interesting changes - for detailed changelogs, consult the release notes for individual snapshots below.

This release does *not* preserve compatibility with third-party plugins written for NetLink 1.x!
...
- **Added TLS certificate verification, which is enabled by default on Clientbot networks**. [issue#592](https://github.com/jlu5/NetLink/issues/592)
...
- New Relay features:
    - LINKACL now supports whitelisting networks in addition to the original blacklist implementation (see `help LINKACL`). [issue#394](https://github.com/jlu5/NetLink/issues/394)
    ...
    - KILL handling received a major rework ([issue#520](https://github.com/jlu5/NetLink/issues/520):
    ...
    - `relay_clientbot` now supports setting clientbot styles by network. [issue#455](https://github.com/jlu5/NetLink/issues/455)
    - The defaults for CLAIM (on or off) and LINKACL (whitelist or blacklist mode) can now be pre-configured for new channels. [issue#581](https://github.com/jlu5/NetLink/issues/581)
    - New `relay::allow_free_oper_links` option allows disabling oper access to `CREATE/LINK/DELINK/DESTROY/CLAIM` by default
    - relay_clientbot: add support for showing prefix modes in relay text, via a new `$mode_prefix` expansion. [issue#540](https://github.com/jlu5/NetLink/issues/540)
- Clientbot is now more featureful:
    ...
    - Configurable alternate / fallback nicks are now supported: look for the `netlink_altnicks` option in the example config.
    ...
    - Failed attempts to join channels are now logged to warning. [issue#533](https://github.com/jlu5/NetLink/issues/533)
...
- Messages sent by most commands are now transparently word-wrapped to prevent cutoff. [issue#153](https://github.com/jlu5/NetLink/issues/153)
- NetLink accounts are now implicitly matched: i.e. `user1` is now equivalent to `$netlinkacc:user1`.
- NetLink now responds to remote `/STATS` requests (`/stats c`, `u`, and `o`) if the `stats` plugin is loaded.
- The NetLink service client no longer needs to be in channels to log to them.

#### Feature changes

- **The ratbox protocol module has been merged into ts6**, with a new `ircd: ratbox` option introduced to declare Ratbox as the target IRCd. [issue#543](https://github.com/jlu5/NetLink/issues/543)
- The `raw` command has been split into a new plugin (`plugins/raw.py`) with two permissions: `raw.raw` for Clientbot networks, and `raw.raw.unsupported_network` for other protocols. Using raw commands outside Clientbot is not supported. [issue#565](https://github.com/jlu5/NetLink/issues/565)
...
- Fix `irc.parse_modes()` incorrectly mangling modes changes like `+b-b *!*@test.host *!*@test.host` into `+b *!*@test.host`. [issue#573](https://github.com/jlu5/NetLink/issues/573)
- clientbot: fixed sending duplicate JOIN hooks and AWAY status updates. [issue#551](https://github.com/jlu5/NetLink/issues/551)

#### Internal improvements
- Reading from sockets now uses a select-based backend instead of one thread per network.
- Major optimizations to user tracking that lets NetLink handle Relay networks of 500+ users.
    - This is done via a new `UserMapping` class in `netlink.classes`, which stores User objects by UID and provides a `bynick` attribute mapping case-normalized nicks to lists of UIDs.
...
- Service bot handling was completely redone to minimize desyncs when mixing Relay and services. [issue#265](https://github.com/jlu5/NetLink/issues/265)
...
- Rewrote the Relay Quick Start Guide. [issue#619](https://github.com/jlu5/NetLink/issues/619)
- FAQ: expanded Relay section with common questions regarding Relay mechanics (i.e. kill, mode, and server bans handling)
- docs/technical, docs/permissions-reference: many updates to bring up to date with NetLink 2.0
- various: updated the GitHub repository address

# NetLink 2.0-beta1 (2018-06-27)

This release contains all changes from 2.0-alpha3 as well as the following:

#### New features
- **Added TLS certificate verification, which is now enabled by default on Clientbot networks**. [issue#592](https://github.com/jlu5/NetLink/issues/592)
    - This adds the options `ssl_validate_hostname` and `ssl_accept_invalid_certs` options which have defaults as follows:
        - | Server type              | `ssl_validate_hostname`                             | `ssl_accept_invalid_certs` |
          |--------------------------|-----------------------------------------------------|----------------------------|
          | Full links (S2S)         | false (implied by `ssl_accept_invalid_certs: true`) | true                       |
...
- New Relay features:
    - **LINKACL now supports whitelisting networks in addition to the original blacklist implementation** (see `help LINKACL`). [issue#394](https://github.com/jlu5/NetLink/issues/394)
    - relay: The defaults for CLAIM (on or off) and LINKACL (whitelist or blacklist mode) can now be pre-configured for new channels. [issue#581](https://github.com/jlu5/NetLink/issues/581)
    - You can now set descriptions for channels in `LINKED` via the `CHANDESC` command. [issue#576](https://github.com/jlu5/NetLink/issues/576)
    - `relay_clientbot` now supports setting clientbot styles by network. [issue#455](https://github.com/jlu5/NetLink/issues/455)
    - New `relay::allow_free_oper_links` option allows disabling oper access to `CREATE/LINK/DELINK/DESTROY/CLAIM` by default
- New Antispam features (see the `antispam:` example block for configuration details):
    - Antispam now supports text filtering with configured bad strings. [issue#359](https://github.com/jlu5/NetLink/issues/359)
    - Added `block` as a punishment to only hide messages from other plugins like relay. [issue#616](https://github.com/jlu5/NetLink/issues/616)
    - Antispam can now process PMs to NetLink clients for spam - this can be limited to service bots, enabled for all NetLink clients (including relay clones), or disabled entirely (the default).
    - IRC formatting (bold, underline, colors, etc.) is now removed before processing text. [issue#615](https://github.com/jlu5/NetLink/issues/615)
- IPv4/IPv6 address selection is now automatic, detecting when an IPv6 address or bindhost is given. [issue#212](https://github.com/jlu5/NetLink/issues/212)
- Messages sent by most commands are now transparently word-wrapped to prevent cutoff. [issue#153](https://github.com/jlu5/NetLink/issues/153)
- The Global plugin now supports configuring exempt channels. [issue#453](https://github.com/jlu5/NetLink/issues/453)
- Automode now allows removing entries by entry numbers. [issue#506](https://github.com/jlu5/NetLink/issues/506)
...
- Reworked relay+clientbot op checks on to [be more consistent](https://github.com/jlu5/NetLink/compare/fee64ece045ad9dc49a07d1b438caa019c90a778~...d4bf407).
- inspircd: fix potential desyncs when sending a kill by removing the target immediately. [issue#607](https://github.com/jlu5/NetLink/issues/607)
- UserMapping: fixed a missing reference to the parent `irc` instance causing errors on nick collisions.
- clientbot: suppress warnings if `/mode #channel` doesn't show arguments to `+lk`, etc. [issue#537](https://github.com/jlu5/NetLink/issues/537)
...
- opercmds: forbid killing the main NetLink client.

#### Internal changes
- Login handling was rewritten and moved entirely from `coremods.corecommands` to `coremods.login`. [issue#590](https://github.com/jlu5/NetLink/issues/590)
- New stuff for the core network classes:
    ...
    - `User`, `Channel` TS values are now consistently stored as `int`. [issue#594](https://github.com/jlu5/NetLink/issues/594)
...
- New stuff in utils:
    - `remove_range()` removes a range string of (one-indexed) items from the list, where range strings are indices or ranges of them joined together with a "," (e.g. "5", "2", "2-10", "1,3,5-8") - [issue#506](https://github.com/jlu5/NetLink/issues/506)
    - `get_hostname_type()` takes in an IP or hostname and returns an int representing the detected address type: 0 (none detected), 1 (IPv4), 2 (IPv6) - [issue#212](https://github.com/jlu5/NetLink/issues/212)
    - `parse_duration()` takes in a duration string (in the form `1w2d3h4m5s`, etc.) and returns the equiv. amount of seconds - [issue#504](https://github.com/jlu5/NetLink/issues/504)
- The TLS/SSL setup bits in `IRCNetwork` were broken into multiple functions: `_make_ssl_context()`, `_setup_ssl`, and `_verify_ssl()`
- Removed deprecated attributes: `irc.botdata`, `irc.conf`, `utils.is*` methods, `NetLinkNetworkCoreWithUtils.check_authenticated()` - [issue#422](https://github.com/jlu5/NetLink/issues/422)
- NetLinkNCWUtils: The `allowAuthed`, `allowOper` options in `is_oper()` are now deprecated no-ops (they are set to False and True respectively)

# NetLink 2.0-alpha3 (2018-05-10)

This release contains all changes from 1.3.0, as well as the following:

#### New features
- **Experimental daemonization support via `netlink -d`**. [issue#187](https://github.com/jlu5/NetLink/issues/187)
- New (alpha-quality) `antispam` plugin targeting mass-highlight spam: it supports any combination of kick, ban, quiet (mute), and kill as punishment. [issue#359](https://github.com/jlu5/NetLink/issues/359)
- Clientbot now supports expansions such as `$nick` in autoperform.
- Relay now translates STATUSMSG messages (e.g. `@#channel` messages) for target networks instead of passing them on as-is. [issue#570](https://github.com/jlu5/NetLink/issues/570)
...
#### Feature changes
- **Reverted the commit making SIGHUP shutdown the NetLink daemon**. Now, SIGUSR1 and SIGHUP both trigger a rehash, while SIGTERM triggers a shutdown.
- The `raw` command has been split into a new plugin (`plugins/raw.py`) with two permissions: `raw.raw` for Clientbot networks, and `raw.raw.unsupported_network` for other protocols. Using raw commands outside Clientbot is not supported. [issue#565](https://github.com/jlu5/NetLink/issues/565)
...
- relay: fix KeyError when a local client is kicked from a claimed channel. [issue#572](https://github.com/jlu5/NetLink/issues/572)
- Fix `irc.parse_modes()` incorrectly mangling modes changes like `+b-b *!*@test.host *!*@test.host` into `+b *!*@test.host`. [issue#573](https://github.com/jlu5/NetLink/issues/573)
...
- Fixed various 2.0-alpha2 regressions:
    ...
    - protocols/inspircd: fix supported modules list being corrupted when an indirectly linked server shuts down. [issue#567](https://github.com/jlu5/NetLink/issues/567)
- networks: `remote` now properly errors if the target service is not available on a network. [issue#554](https://github.com/jlu5/NetLink/issues/554)
...
- **API Break: significantly reworked channel handling for service bots**. [issue#265](https://github.com/jlu5/NetLink/issues/265)
    ...
    - Persistent channels are also "dynamic" in the sense that NetLink service bots will now part channels marked persistent when they become empty, and rejoin when it is recreated.
    ...
    - New abstraction: `ServiceBot.part()` sends a part request to channels and only succeeds if it is not marked persistent by any plugin. This effectively works around the long-standing issue of relay-services conflicts. [issue#265](https://github.com/jlu5/NetLink/issues/265)
- Major optimizations to `irc.nick_to_uid`: `NetLinkNetworkCore.users` and `classes.User` now transparently maintain an index mapping nicks to UIDs instead of doing reverse lookup on every call.
    - This is done via a new `UserMapping` class in `netlink.classes`, which stores User objects by UID and provides a `bynick` attribute mapping case-normalized nicks to lists of UIDs.
    - `classes.User.nick` is now a property, where the setter implicitly updates the `bynick` index with a pre-computed case-normalized version of the nick (also stored to `User.lower_nick`)
- Various relay optimizations: reuse target SID when bursting joins, and only look up nick once in `normalize_nick`
- Rewritten CTCP plugin, now extending to all service bots. [issue#468](https://github.com/jlu5/NetLink/issues/468), [issue#407](https://github.com/jlu5/NetLink/issues/407)
...
- New API: hook handlers can now filter messages from lower-priority handlers by returning `False`. [issue#547](https://github.com/jlu5/NetLink/issues/547)
- New API: added `irc.get_server_option()` to fetch server-specific config variables and global settings as a fallback. [issue#574](https://github.com/jlu5/NetLink/issues/574)
- automode: replace assert checks with proper exceptions
- Renamed methods in log, utils, conf to snake case. [issue#523](https://github.com/jlu5/NetLink/issues/523)
...
- relay: added new option `server::<networkname>:relay_forcetag_nicks`, a per-network list of nick globs to always tag when introducing users onto a network. [issue#564](https://github.com/jlu5/NetLink/issues/564)
- Added support for more channel modes in Relay:
    * blockcaps: inspircd +B, elemental-ircd +G
    * exemptchanops: inspircd +X
    * filter: inspircd +g, unreal extban ~T:block ([issue#557](https://github.com/jlu5/NetLink/issues/557))
    * hidequits: nefarious +Q, snircd +u
    * history: inspircd +H
    * largebanlist: ts6 +L
    * noamsg: snircd/nefarious +T
    * blockhighlight: inspircd +V (extras module)
    * kicknorejoin: elemental-ircd +J ([issue#559](https://github.com/jlu5/NetLink/issues/559))
    * kicknorejoin_insp: inspircd +J (with argument; [issue#559](https://github.com/jlu5/NetLink/issues/559))
    * repeat: elemental-ircd +E ([issue#559](https://github.com/jlu5/NetLink/issues/559))
    * repeat_insp: inspircd +K (with argument; [issue#559](https://github.com/jlu5/NetLink/issues/559))
- Added support for UnrealIRCd extban `~T` in Relay. [issue#557](https://github.com/jlu5/NetLink/issues/557)
...
#### Bug fixes
- Fix default permissions not applying on startup (2.0-alpha1 regression). [issue#542](https://github.com/jlu5/NetLink/issues/542)
- Fix rejoin-on-kill for the main NetLink bot not working (2.0-alpha1/[94e05a6](https://github.com/jlu5/NetLink/commit/94e05a623314e9b0607de4eb01fab28be2e0c7e1) regression).
- Clientbot fixes:
    - Fix desyncs caused by incomplete nick collision checking when a user on a Clientbot link changes their nick to match an existing virtual client. [issue#535](https://github.com/jlu5/NetLink/issues/535)
    - Fix desync involving ghost users when a person leaves a channel, changes their nick, and rejoins. [issue#536](https://github.com/jlu5/NetLink/issues/536)
...
- Fix long standing issues where relay would sometimes burst users multiple times on connect. [issue#529](https://github.com/jlu5/NetLink/issues/529)
    - Also fix a regression from 2.0-alpha1 where users would not be joined if the hub link is down ([issue#548](https://github.com/jlu5/NetLink/issues/548))
- Fix `$a:account` extbans being dropped by relay (they were being confused with `$a`). [issue#560](https://github.com/jlu5/NetLink/issues/560)
- Fix corrupt arguments when mixing the `remote` and `mode` commands. [issue#538](https://github.com/jlu5/NetLink/issues/538)
- Fix lingering queue threads when networks disconnect. [issue#558](https://github.com/jlu5/NetLink/issues/558)
...
- protocols/ts6 no longer requires `SAVE` support from the uplink. [issue#545](https://github.com/jlu5/NetLink/issues/545)
- ts6, hybrid: miscellaneous cleanup
- protocols/inspircd now tracks module (un)loads for `m_chghost.so` and friends. [issue#555](https://github.com/jlu5/NetLink/issues/555)
- Clientbot now logs failed attempts in joining channels. [issue#533](https://github.com/jlu5/NetLink/issues/533)

# NetLink 2.0-alpha1 (2017-10-07)
...
- Clientbot is now more featureful:
    ...
    - Clientbot can now optionally sync ban lists when it joins a channel, allowing Relay modesync to unset bans properly. See the `fetch_ban_lists` option in the example config.
- NetLink received a new launcher, which now checks for stale PID files (when `psutil` is installed) and supports shutdown/restart/rehash via the command line.
- New commands for the opercmds plugin, including:
    - `chghost`, `chgident`, and `chgname`, for IRCds that don't expose them as commands.
    - `massban`, `masskill`, `massbanre`, and `masskillre` - these commands allow setting kickbans, kills, or glines on users matching a NetLink mask (`n!u@h` mask/exttarget) or regular expression. The hope is that these tools can help opers actively fight botnets as they are connected, similar to atheme's `clearchan` and Anope's `chankill` commands.
...
- NetLink's connection time is now displayed when WHOISing service bots. This info can be turned off using the `netlink:whois_show_startup_time` option.
...
#### Backwards incompatible changes
- Signal handling on Unix was updated to use `SIGUSR1` for rehash and `SIGHUP` for shutdown - this changes NetLink to be more in line with foreground programs, which generally close with the owning terminal.
- Some options were deprecated and renamed:
    - The `p10_ircd` option for P10 servers is now named `ircd`, though the old option will still be read from.
    - The `use_elemental_modes` setting on ts6 networks has been deprecated and replaced with an `ircd` option targeting charybdis, elemental-ircd, or chatircd. Supported values for `ircd` include `charybdis`, `elemental`, and `chatircd`.
- PID file checking is now enabled by default, along with checks for stale PID files *only* when [`psutil`](https://pythonhosted.org/psutil/) is installed. Users upgrading from NetLink < 1.1-dev without `psutil` installed will need remove NetLink's PID files before starting the service.
- The `fml` command in the `games` plugin was removed.
...
#### Internal changes
- **API Break**: The protocol module layer is completely rewritten, with the `Irc` and `Protocol`-derived classes combining into one. Porting **will** be needed for old protocol modules and plugins targetting 1.x; see the [new (WIP) protocol specification](https://github.com/jlu5/NetLink/blob/devel/docs/technical/pmodule-spec.md) for details.
...
- Errors due to missing permissions now log to warning. [issue#593](https://github.com/jlu5/NetLink/issues/593)
- Documentation updates to advanced-relay-config.md and the FAQ

# NetLink 1.3-beta1 (2018-04-07)
...
- **Backported the launcher from 2.0-alpha2**:
    - Added support for daemonization via the `--daemon/-d` option. [issue#187](https://github.com/jlu5/NetLink/issues/187)
    - Added support for shutdown/restart/rehash via the command line. [issue#244](https://github.com/jlu5/NetLink/issues/244)
    - The launcher now detects and removes stale PID files when `psutil` (an optional dependency) is installed, making restarting from crashes a more streamlined process. [issue#512](https://github.com/jlu5/NetLink/issues/512)
    - PID file checking is now enabled by default, with the `--check-pid/-c` option retained as a no-op option for compatibility with NetLink <= 1.2
    - Following 2.0 changes, sending SIGUSR1 to the NetLink daemon now triggers a rehash (along with SIGHUP).
- Service bot idents, hosts, and realnames can now be configured globally and on a per-network basis. [issue#281](https://github.com/jlu5/NetLink/issues/281)
- Relay server suffix is now configurable by network (`servers::<netname>::relay_server_suffix` option). [issue#462](https://github.com/jlu5/NetLink/issues/462)
- Login blocks can now be restricted to specific networks, opered users, and hostmasks. [issue#502](https://github.com/jlu5/NetLink/issues/502)
...
#### Misc changes
- Config loading now uses `yaml.safe_load()` instead of `yaml.load()` so that arbitrary code cannot be executed. [issue#589](https://github.com/jlu5/NetLink/issues/589)
...
- Fix commonly reported crashes on `logging:` config syntax errors ([49136d5](https://github.com/jlu5/NetLink/commit/49136d5abd609fd5e3ba2ec2e42a0443118e62ab))
...
- Fix extraneous removal of PID files even when `-n/--no-check-pid` was set or if the PID file already existed on launch (NetLink would quit and delete the old PID file in these cases)
...
- p10: fix rejoin-on-kick relaying by also acknowledging kicks sent from NetLink to its clients with a PART
...
- **Certain configuration options were renamed / deprecated:**
    - The `bot:` configuration block was renamed to `netlink:`, with the old name now deprecated.
...
- **Using new-style accounts exclusively (i.e. specifying things under `login:accounts`) now requires a `permissions:` block for NetLink to start** (#413).
...
- Plugins and protocol modules can now be loaded from any directory, using the new `plugin_dirs` and `protocol_dirs` options. (#350)
    - The `netlink-contribdl` utility was dropped as it is now it obsolete.
...
- The `remote` command now routes replies back to the caller network, and supports service bots that aren't the main NetLink bot via a `--service` option.
- Service bot spawning can now be disabled via the global `spawn_services` and per-service `spawn_service` options; doing so will merge the relevant commands into the main NetLink bot.
- Added a `netlink::show_unknown_commands` option to optionally disable "unknown command" errors. This option can also be overridden per-server and per-service (#441).
- Added a `netlink::prefer_private_replies` option to default to private command replies (#409).
...
#### Feature changes
- Added configurable encoding support via the `encoding` option in server config blocks ([#467](https://github.com/jlu5/NetLink/pull/467)).
- **Certain configuration options were renamed / deprecated:**
    - The `bot:` configuration block was renamed to `netlink:`, with the old name now deprecated.
    - `logging:stdout` is now `logging:console` (the previous name was a misnomer since text actually went to `stderr`).
    - The `bot:prefix` option is deprecated: you should instead define the `prefixes` setting in a separate config block for each service you wish to customize (e.g. set `automode:prefix` and `games:prefix`)
- Added new `$and` and `$network` exttargets - see the new [exttargets documentation page](https://github.com/jlu5/NetLink/blob/1.2-beta1/docs/exttargets.md) for how to use them.
...
- `relay_clientbot` no longer colours network names by default. It is still possible to restore the old behaviour by defining [custom clientbot styles](https://github.com/jlu5/NetLink/blob/1.2-beta1/docs/advanced-relay-config.md#custom-clientbot-styles).
...
- servprotect: only track kills and saves to NetLink clients, not all kills on a network!
- Fix `~#channel` prefix messages not working over relay on RFC1459-casemapping networks ([#464](https://github.com/jlu5/NetLink/issues/464)).
...
- Plugins and protocol modules can now be loaded from any directory, making netlink-contrib 20% more useful! Look for the `plugin_dirs` and `protocol_dirs` options in the example config. (#350)
    - The `netlink-contribdl` utility was dropped as this makes it obsolete.
- Relay now forbids linking to channels where the home network is down, which previously caused errors due to unreliable target TS (#419). This, along with the regular TS check, can be overridden with a new `link --force` option.
- SASL timeout for clientbot is now configurable, with the default raised to 15 seconds
- **New-style accounts (i.e. anything under `login:accounts`) now require a `permissions:` block for NetLink to start** (#413).
- New `stats` (uptime info) and `global` (global announcement) plugins
- Improvements to the `remote` command of the `networks` plugin:
    - Plugin replies are now routed back to the sender's network.
    - Added an optional `--service` option to call commands for service bots other than the main NetLink service.
- `servprotect` KILL/SAVE limits and expiry time are now configurable (#417)
- Added the `netlink::show_unknown_commands` option to optionally disable "unknown command" errors. This option can also be overridden per-server and per-service (#441).
- Added the `netlink::prefer_private_replies` option to default to private command replies (#409)
...
- NetLink protocol capabilities were introduced, reducing the amount of protocol name hardcoding in plugins (#337)
...
#### Misc changes
- The `bot:` config block is renamed to `netlink:`. Existing configurations will still work, as the `bot` and `netlink` blocks are always merged together (#343).
- Added better documentation for generic options available to all service bots (`nick`, `ident`, `prefix` for fantasy, etc.)
- `Irc.botdata`, `Irc.conf`, `Irc.checkAuthenticated()` have been deprecated: use `conf.conf['netlink']`, `conf.conf`, and the permissions API respectively instead!
...
#### Bug fixes
- The `netlink-mkpasswd` program is actually installed now when using `pip` (PyPI) or `setup.py`.
...
#### Feature changes
- `netlink-mkpasswd` now supports interactive password input via getpass, which is more secure than passing passwords raw on the command line.
...
- Added utilities to download contrib modules (`netlink-contribdl`) and hash passwords for the new accounts configuration (`netlink-mkpasswd`).
...
- PID file checking was implemented - Run NetLink with the `--check-pid` argument in order to turn it on.
...
- New `servermaps` plugin, displaying network `/map`'s from the NetLink server's perspective.
...
The "Crunchy" release. This release includes all bug fixes from NetLink 1.0.4, along with the following:
...
- Most plugins were ported to use the permissions API, meaning multiple accounts are actually useful now!
    - Having a **NetLink login** with *new* style accounts **no longer implies access to everything** on the NetLink daemon. See the updated example conf regarding the new `permissions:` block.
...
- PID file checking is now **disabled** by default for better migration from 1.0.x. Run NetLink with `--check-pid` in order to turn it on.
...
#### Bug fixes
- protocols: implement basic nick collision detection in UID handlers. This fixes users potentially going missing on UnrealIRCd networks, if the remote IRCd introduces a user of the same nick *after* NetLink does. Thanks to kevin for reporting!
...
- PID file checking has been implemented and is turned on by default. :warning: **When upgrading from 1.0.x, remove `netlink.pid` before starting!**
- Revamped login configuration system, which now supports **multiple accounts and hashed passwords**
    ...
    - Hashing new passwords can be done using the `mkpasswd` command via IRC, or the new command line utility `netlink-mkpasswd`.
- Services bots now support setting modes on themselves on join: see "joinmodes" in the example conf
- Added a contrib module downloader (`netlink-contribdl`)
- Relay now uses the NetLink permission system. The following permissions are now available:
...
- New servermaps plugin, displaying network `/map`'s from the NetLink server's perspective. Relay links can be expanded as well to make this more detailed!
...
- The NetLink launcher now sets the terminal title on launch (Windows & POSIX)
- relay: implement a complementary `showchan` command, showing a list of relay links

#### Bug fixes
- Relay is potentially less crashy when NetLink is split off while it tries to introduce users (#354).
...
- Removed `update.sh` (my convenience script for locally building + running NetLink)

# [NetLink 1.0.3](https://github.com/jlu5/NetLink/releases/tag/1.0.3)
...
#### Bug fixes
- Fixed invalid database names such as "automode-/tmp/test.db" from being generated when NetLink is started with an absolute path to its config.
...
# [NetLink 1.0.2](https://github.com/jlu5/NetLink/releases/tag/1.0.2)
...
#### Feature changes
- Clientbot now uses a more specific realname fallback ("NetLink Relay Mirror Client") instead of potentially misleading text such as "NetLink Service Client". In the future, this text may be made configurable.
...
# [NetLink 1.0.1](https://github.com/jlu5/NetLink/releases/tag/1.0.1)
...
- **Fix NetLink being uninstallable via PyPI due to missing VERSION and README.md**
...
# [NetLink 1.0.0](https://github.com/jlu5/NetLink/releases/tag/1.0.0)
...
# [NetLink 1.0-beta1](https://github.com/jlu5/NetLink/releases/tag/1.0-beta1)
...
- Irc: implement basic message queueing (1 message sent per X seconds, where X defaults to 0.01 for servers) .
    - This appears to also workaround sporadic SSL errors causing disconnects (https://github.com/jlu5/NetLink/issues/246)
...
- The NetLink launcher now shows the full VCS version in `netlink -v`
...
# [NetLink 0.10-alpha1](https://github.com/jlu5/NetLink/releases/tag/0.10-alpha1)
Tagged as **0.10-alpha1** by [jlu5](https://github.com/jlu5) on 2016-08-22T00:04:34Z

The "Balloons" release. Note: This is an **alpha** build and may not be completely stable! This version includes all fixes from NetLink 0.9.2, with the following additions:
...
- **WIP** Clientbot protocol module: allows NetLink to connect as a bot to servers, for purposes such as relay.
...
- Ctrl-C / KeyboardInterrupt now cleanly shuts down NetLink (in most cases).
...
#### Internal fixes / improvements
- Shutdown now cleanly quits the NetLink service bot instead of simply splitting off.
- NetLink now shows a better error if a protocol module chosen is missing.
- Config key validation is now protocol-specific.
- The `IrcUser.identified` attribute was renamed to `IrcUser.account`.
- exec: make `netlink` and `importlib` accessible for easier debugging.
- SQUIT hooks get a few more arguments, such as `nicks` (affected nicks) and `serverdata` (old IrcServer object).
- Retrieving the hostname used by the current NetLink instance is now a shared function: `irc.hostname()`
...
# [NetLink 0.9.2](https://github.com/jlu5/NetLink/releases/tag/0.9.2)
...
- Fixed services login tracking for older Anope services + UnrealIRCd. Previously, NetLink would incorrectly store login timestamps as the account name, instead of the user's nick.
...
# [NetLink 0.9.1](https://github.com/jlu5/NetLink/releases/tag/0.9.1)
...
- The configuration file is now **netlink.yml** by default, instead of **config.yml**.
- NetLink now requires installing itself as a module, instead of simply running from source. Do this via `python3 setup.py install --user`.
...
# [NetLink 0.9.0](https://github.com/jlu5/NetLink/releases/tag/0.9.0)
...
- NetLink is now slightly more descriptive if you try to start it with missing dependencies or a missing conf file.
- The NetLink API reference is now at https://netlink.github.io/ instead of in `docs/technical`.
- The `exec` and `eval` commands now have access to the `netlink` and `importlib` imports by default.
...
- Documentation update: there are now guides to Automode and NetLink's Services API.

##### Bug fixes
- Fixed the `reload` command (again).
- Fixed compatibility with ircmatch 1.2: NetLink previously used features that were only available in the unreleased Git version.
...
- `update.sh` now passes arguments to the `netlink` launcher.

# [NetLink 0.9-beta1](https://github.com/jlu5/NetLink/releases/tag/0.9-beta1)
...
- WHOIS requests to the NetLink server for clients that aren't NetLink bots now work (syntax: `/raw whois netlink.server somenick`). "No such nick" errors are also sent when the target is missing, instead of raising warnings.
    - These WHOIS responses were previously sent from the wrong source (the server that the client was on, instead of the NetLink server), causing them to be ignored if they were going the wrong way.
...
# [NetLink 0.9-alpha1](https://github.com/jlu5/NetLink/releases/tag/0.9-alpha1)
Tagged as **0.9-alpha1** by [jlu5](https://github.com/jlu5) on 2016-07-09T07:27:47Z

### Summary of changes from 0.8.x

##### Backwards incompatible changes
 - Configuration file is now **netlink.yml** by default, instead of **config.yml**.
 - NetLink now requires installing itself as a module, instead of simply running from source. Do this via `python3 setup.py install --user`.
...
- New exttarget support: see https://github.com/jlu5/NetLink/blob/0.9-alpha1/coremods/exttargets.py#L15 for a list of supported ones.
...
- `--version`, `--no-pid`, and `--help` are now implemented by NetLink's command line script
...
- protocols/inspircd: new `use_experimental_whois` server option, which forces NetLink to handle WHOIS requests locally. This allows relay WHOIS extensions like account name and origin server name (`whois_show_*` options) to actually work on InspIRCd networks.
- NetLink can now optionally protect its services clients by setting servprotect modes (InspIRCd umode `+k`, `+S` elsewhere). The option for this is `protect_services` in the `bot:` block.
...
- The NetLink bot now rejoins relay channels when it is killed
...
# [NetLink 0.8-alpha4](https://github.com/jlu5/NetLink/releases/tag/0.8-alpha4)
...
Full diff: https://github.com/jlu5/NetLink/compare/0.8-alpha3...0.8-alpha4

# [NetLink 0.8-alpha3](https://github.com/jlu5/NetLink/releases/tag/0.8-alpha3)
...
- The main NetLink client now spawns itself with hideoper whenever available, to avoid filling up `/lusers` and `/stats P/p`. (#194)
- The `fantasy` plugin now supports per-bot prefixes; see example conf for details.
- Purge c_ and u_ prefixes from NetLink's internal named modes definitions (#217).
...
# [NetLink 0.8-alpha2](https://github.com/jlu5/NetLink/releases/tag/0.8-alpha2)
...
[Full diff](https://github.com/jlu5/NetLink/compare/0.8-alpha1...0.8-alpha2)

# [NetLink 0.8-alpha1](https://github.com/jlu5/NetLink/releases/tag/0.8-alpha1)
...
- SIGHUP now rehashes NetLink on supported platforms (#179)
- Improved mode support for Charybdis (#203)
- Fix disconnect logic during ping timeouts

[Full diff](https://github.com/jlu5/NetLink/compare/0.7.2-dev...0.8-alpha1)

# [NetLink 0.7.2-dev](https://github.com/jlu5/NetLink/releases/tag/0.7.2-dev)
...
- 13e97177e2641595709a2940eb038b9fac2bdb3a docs: Add a NetLink oper guide
...
- cf363432f0aa9d8152cfbe2bbbdb6f644bcef960 netlink: use abspath() to get the source directory
...
- 4125ff33b1803ab12e475fa508b3d1e1578f38b4 netlink: prettier "Loaded plugins" log message on start
...
- fbd8659a7d6e2b08b7baa45bbdd95dbff67724b4 classes: spawn NetLink clients with a custom opertype
...
# [NetLink 0.7.1-dev](https://github.com/jlu5/NetLink/releases/tag/0.7.1-dev)
...
# [NetLink 0.7.0-dev](https://github.com/jlu5/NetLink/releases/tag/0.7.0-dev)
...
# [NetLink 0.6.1-dev](https://github.com/jlu5/NetLink/releases/tag/0.6.1-dev)
...
# [NetLink 0.6.0-dev](https://github.com/jlu5/NetLink/releases/tag/0.6.0-dev)
...
- Some core changes will **break** protocol modules and plugins written for older NetLink versions:
...
- Various bug fixes - see https://github.com/jlu5/NetLink/compare/0.5-dev...0.6.0-dev for a full diff.

# [NetLink 0.5-dev](https://github.com/jlu5/NetLink/releases/tag/0.5-dev)
...
- fantasy: Support nick prefixes (e.g. `NetLink: help`), along with a configurable prefix.
...
Full diff:https://github.com/jlu5/NetLink/compare/0.4.6-dev...0.5-dev

# [NetLink 0.4.6-dev](https://github.com/jlu5/NetLink/releases/tag/0.4.6-dev)
...
# [NetLink 0.4.5-dev](https://github.com/jlu5/NetLink/releases/tag/0.4.5-dev)
...
Full diff since 0.4.0-dev: https://github.com/jlu5/NetLink/compare/0.4.0-dev...0.4.5-dev

# [NetLink 0.3.50-dev](https://github.com/jlu5/NetLink/releases/tag/0.3.50-dev)
...
- 3d621b0 Move checkAuthenticated() to utils, and give it and isOper() toggles for allowing oper/NetLink logins
- 090fa85 Move Irc() from main.py to classes.py

# [NetLink 0.3.1-dev](https://github.com/jlu5/NetLink/releases/tag/0.3.1-dev)
Tagged as **0.3.1-dev** by [jlu5](https://github.com/jlu5) on 2015-09-03T06:56:48Z

Bugfix release + LINKACL support for relay. [Commits since 0.3.0-dev](https://github.com/jlu5/NetLink/compare/0.3.0-dev...0.3.1-dev):
...
# [NetLink 0.2.5-dev](https://github.com/jlu5/NetLink/releases/tag/0.2.5-dev)
Tagged as **0.2.5-dev** by [jlu5](https://github.com/jlu5) on 2015-08-16T05:39:34Z

See the diff for this development build: https://github.com/jlu5/NetLink/compare/0.2.3-dev...0.2.5-dev

# [NetLink 0.2.3-dev](https://github.com/jlu5/NetLink/releases/tag/0.2.3-dev)
Tagged as **0.2.3-dev** by [jlu5](https://github.com/jlu5) on 2015-07-26T06:11:20Z

The "prevent NetLink from wrecking my server's CPU" release.

Mostly bug fixes here, with a couple of scripts added (`start-cpulimit.sh` and `kill.sh`) added to assist running NetLink under the protection of [CPUlimit](https://github.com/opsengine/cpulimit). :)

#### New features

- relay: Block most duplicate modes from being relayed (#71)
- main: write a PID file to `netlink.pid` (f85fbd934bb2001122079da9e37e2fd1f9041a18)
...
Full diff: https://github.com/jlu5/NetLink/compare/0.2.2-dev...0.2.3-dev

# [NetLink 0.2.2-dev](https://github.com/jlu5/NetLink/releases/tag/0.2.2-dev)
...
Full diff: https://github.com/jlu5/NetLink/compare/0.2.0-dev...0.2.2-dev

# [NetLink 0.2.0-dev](https://github.com/jlu5/NetLink/releases/tag/0.2.0-dev)
...
Full diff: https://github.com/jlu5/NetLink/compare/0.1.6-dev...0.2.0-dev

# [NetLink 0.1.6-dev](https://github.com/jlu5/NetLink/releases/tag/0.1.6-dev)
...
- Log output to `log/netlink.log`, along with console (#52, 61804b1 fbc2fbf)
- main/coreplugin: use `log.exception()` instead of `traceback.print_exc()`, so logs aren't only printed to screen (536366d)
- relay: don't relay messages sent to the NetLink client (688675d)
...
Full diff: https://github.com/jlu5/NetLink/compare/0.1.5-dev...0.1.6-dev

# [NetLink 0.1.5-dev](https://github.com/jlu5/NetLink/releases/tag/0.1.5-dev)
...
- New `NETLINK_DISCONNECT` hook to keep track of network disconnections (3f6f78b)
- relay: add a `LINKED` command (#65, bbcd70b)
- commands: add a `HELP` command (#8, 6508cb3 4553eda)

### Bug fixes
- relay: don't spawn tagged clones for the internal NetLink client (40fd9e3)
...
- relay: only join NetLink to channels & set topics if there's actually a relay on the network (49943a7)
...
You can view the full diff here: https://github.com/jlu5/NetLink/compare/0.1.0-dev...0.1.5-dev

# [NetLink 0.1.0-dev](https://github.com/jlu5/NetLink/releases/tag/0.1.0-dev)
Tagged as **0.1.0-dev** by [jlu5](https://github.com/jlu5) on 2015-07-16T06:27:12Z

NetLink's first pre-alpha development snapshot.
